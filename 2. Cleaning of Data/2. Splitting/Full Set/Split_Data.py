...
###############################################################
#Data Frame storing actual values with new variable names /not really needed
...
# May need to remove \ to run code placed in code so it can presented in paper

Epoch_train, Epoch_test, Temp_train, Temp_test,\
Ic_train,Ic_test,\
Vc_train,Vc_test\
 = train_test_split( Time_Epoch, Package_Temp,Ic,Vc,test_size = 0.2, shuffle = False)

Ig_train,Ig_test,Vg_train,Vg_Test,\
Rc_train,Rc_test,\
Rg_train,Rg_test,Pc_train,Pc_test\
 = train_test_split(Ig,Vg,Rc,Rg,Pc,test_size=0.2,shuffle=False )
...
Train_1.to_pickle('Full_Train_Data.pkl')
Test_1.to_pickle('Full_Test_Data.pkl')
